...
function generatePnPPowerShell(input: ISchemaGeneratorInput): string {
  const lines: string[] = [
    ...
    '# Focused on developer-relevant settings and customizations',
    '',
    '# Connect to site',
  ...
  if (input.lists.length > 0) {
    ...
    input.lists.forEach(list => {
      ...
      lines.push(`New-PnPList -Title "${escapePowerShellString(list.title)}" -Template ${template}`);

      if (list.description) {
        lines.push(`Set-PnPList -Identity "${escapePowerShellString(list.title)}" -Description "${escapePowerShellString(list.description)}"`);
      }

      // Add fields
      list.fields.filter(f => !f.readOnly && !isSystemField(f.internalName)).forEach(f => {
        lines.push(`Add-PnPField -List "${escapePowerShellString(list.title)}" -DisplayName "${escapePowerShellString(f.title)}" -InternalName "${escapePowerShellString(f.internalName)}" -Type ${f.fieldType}${f.required ? ' -Required' : ''}`);
      });

      if (list.hasUniqueRoleAssignments && list.roleAssignments.length > 0) {
        lines.push(``);
        lines.push(`# Unique permissions for ${escapePowerShellString(list.title)}`);
        lines.push(`Set-PnPList -Identity "${escapePowerShellString(list.title)}" -BreakRoleInheritance -CopyRoleAssignments:$false`);
        list.roleAssignments.forEach(assignment => {
          assignment.roleDefinitionNames.forEach(roleName => {
            const principalArg = assignment.principalType === 'group'
              ? `-Group "${escapePowerShellString(assignment.principalName)}"`
              : `-User "${escapePowerShellString(assignment.principalName)}"`;
            lines.push(
              `Set-PnPListPermission -Identity "${escapePowerShellString(list.title)}" ${principalArg} -AddRole "${escapePowerShellString(roleName)}"`
            );
          });
        });
      }
...
function generateCliM365(input: ISchemaGeneratorInput): string {
  ...
  if (input.lists.length > 0) {
    ...
    input.lists.forEach(list => {
      ...
      if (list.hasUniqueRoleAssignments && list.roleAssignments.length > 0) {
        lines.push(`# NOTE: ${list.title} has unique permissions. Apply those separately after list creation.`);
      }
    });
    lines.push('');
  }
